"""End-to-end tests of launch(): instantiate wiring + output contract (no real LLM).
...
def test_launch_wiring(cfg_with_mock_llm, cell_prompts):
    """launch() builds the right components, splits train/test, and evaluates on the held-out task."""
    cfg = cfg_with_mock_llm(["name=t", "data=demo", "optimizer=evopromptga", "n_steps=2", "test_frac=0.25"])
    ...
    assert len(test_task.df) == 2  # 25% of the demo dataset's 8 rows held out
    assert isinstance(predictor.llm, MockLLM)


def test_launch_no_split_evaluates_on_train(cfg_with_mock_llm):
    """test_frac=0 -> no held-out split; evaluation runs on the train task."""
    cfg = cfg_with_mock_llm(["name=t", "data=demo", "optimizer=evopromptga", "n_steps=2", "test_frac=0"])
    with patch("promptolution.experiment.launch.evaluate_prompts") as mock_evaluate_prompts:
        mock_evaluate_prompts.return_value = pd.DataFrame({"prompt": ["p"], "score": [1.0]})
        launch(cfg)
...
def test_launch_end_to_end(cfg_with_mock_llm, tmp_path):
    cfg = cfg_with_mock_llm(["name=t", "data=demo", "optimizer=evopromptga", "n_steps=2", "test_frac=0.25"])
    out = launch(cfg)
