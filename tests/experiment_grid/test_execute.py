...
def test_execute_wiring(cfg_with_mock_llm, cell_prompts):
    ...
    cfg = cfg_with_mock_llm(["name=t", "task=dummy", "optimizer=evopromptga", "n_steps=2", "test_frac=0.25"])
    with patch("promptolution.experiment_grid.launch_grid.score_prompts") as mock_score_prompts:
...
def test_execute_no_split_evaluates_on_train(cfg_with_mock_llm):
    ...
    cfg = cfg_with_mock_llm(["name=t", "task=dummy", "optimizer=evopromptga", "n_steps=2", "test_frac=0"])
    with patch("promptolution.experiment_grid.launch_grid.score_prompts") as mock_score_prompts:
        mock_score_prompts.return_value = pd.DataFrame({"prompt": ["p"], "score": [1.0]})
        execute(cfg)
