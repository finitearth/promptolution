...
def test_marker_based_missing_markers():
    ...
    mock_llm = MockLLM(
        predetermined_responses={"Classify: Missing markers": "This response doesn't have the markers at all."}
    )

    classifier = MarkerBasedClassificator(llm=mock_llm, classes=["positive", "neutral", "negative"])
