...
class ClassificationTask(BaseTask):
    ...
    def __init__(
        ...
        if self.n_subsamples is not None:
            self.subsample_strategy = "subsample"
        elif self.block_size is not None:
            self.subsample_strategy = "block"
        else:
            self.subsample_strategy = "full"
    ...
    def subsample(self):
        ...
        """
        if self.subsample_strategy == "full":
            return self.xs, self.ys

        if self.subsample_strategy == "subsample":
            indices = np.random.choice(len(self.xs), self.n_subsamples, replace=False)
            return self.xs[indices], self.ys[indices]

        if self.subsample_strategy == "block":
            block_id = np.random.randint(0, len(self.xs) // self.block_size)  # TODO: optionally sequential block_ids
            indices = np.arange(block_id * self.block_size, (block_id + 1) * self.block_size)
            return self.xs[indices], self.ys[indices]

    def _find_already_evaluated(
    ...
    def evaluate(
        ...
        evaluated, to_evaluate = self._find_already_evaluated(prompts, xs, ys)

        # evaluate the remaining prompts
        prompts_to_evaluate, xs_to_evaluate, _ = zip(*to_evaluate)
